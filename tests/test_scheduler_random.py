...
def make_job(
    ...
    deadline_offset: timedelta = timedelta(hours=12),
...
def test_assign_returns_all_jobs() -> None:
    ...
    result = RandomScheduler().assign(jobs, 0.0, FORECAST)
...
def test_assigned_time_within_feasible_window() -> None:
    ...
    for _ in range(50):
        result = scheduler.assign([job], 0.0, FORECAST)
...
def test_empty_job_list() -> None:
    assert RandomScheduler().assign([], 0.0, FORECAST) == {}


def test_seed_produces_reproducible_results() -> None:
    jobs = [make_job(str(i)) for i in range(5)]
    s1 = RandomScheduler(RandomConfig(seed=42)).assign(jobs, 0.0, FORECAST)
    s2 = RandomScheduler(RandomConfig(seed=42)).assign(jobs, 0.0, FORECAST)
    assert s1 == s2


def test_different_seeds_produce_different_results() -> None:
    jobs = [make_job(str(i)) for i in range(5)]
    s1 = RandomScheduler(RandomConfig(seed=1)).assign(jobs, 0.0, FORECAST)
    s2 = RandomScheduler(RandomConfig(seed=2)).assign(jobs, 0.0, FORECAST)
    assert s1 != s2
